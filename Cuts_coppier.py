...
class Cuts_copy(wx.Frame):
    ...
    def __init__(self,parent,id):
        ...
        if not os.path.exists('Cuts'):
            message=wx.MessageDialog(None,'Nie mogę znaleźć folderu Cuts. Skopiuj folder z przekrojami do folderu głównego i nadaj mu nazwę "Cuts"!','Błąd',wx.ICON_HAND)
            message.ShowModal()
            ...
            self.size_of_sulpture_text_up=wx.StaticText(self.panel, -1, "Rozmiar rzeźby [w mm]", (10,10),(250,-1),wx.ALIGN_CENTER)
            ...
            self.amount_of_cuts_text=wx.StaticText(self.panel, -1, "Ilość wszystkich przekrojów z bazy danych: " + str(amount_of_cuts), (10,110),)
            self.amount_of_cuts_text.SetForegroundColour('white')
            self.amount_of_cuts_text.SetBackgroundColour('dark green')

            self.amount_of_copied_cuts_text=wx.StaticText(self.panel, -1, "Ilość przekrojów jaka została skopiowana: " + str(amount_of_copied_cuts), (10,140),)
    ...
    def Cuts_coppier(self,event):
        ...
        try:
            ...
            self.message_show('Wpisałeś niepoprawne dane!')
            self.Blad_status()
            self.amount_of_copied_cuts_text.SetLabel("Ilość przekrojów jaka została skopiowana: " + str(0))
            return
        if not os.path.exists('Cuts'):
            self.message_show('Nie mogę znaleść folderu Cuts. Skopiuj folder z przekrojami do folderu głównego i nadaj mu nazwę "Cuts"!')
            self.Blad_status()
            self.amount_of_copied_cuts_text.SetLabel("Ilość przekrojów jaka została skopiowana: " + str(0))
            return
        elif size_of_sculpture <= 0 or user_cut_thickness <= 0:
            self.message_show('Muszą to być liczby większe od zera.')
            self.Blad_status()
            self.amount_of_copied_cuts_text.SetLabel("Ilość przekrojów jaka została skopiowana: " + str(0))
            return
            ...
            self.amount_of_copied_cuts_text.SetLabel("Ilość przekrojów jaka została skopiowana: " + str(1))
            ...
            if total_cuts_count == amount_of_cuts:
                while round(cuts_size_count,2) < cut_thickness:
                    ...
                    try:
                        ...
                        self.Blad_status()
                        self.message_show('Bład kopiowania pliku\n'+message_file)
                        return
                    filename_write += 1
                    cuts_size_count += user_cut_thickness
    ...
    def Blad_status(self):
        '''
        Change status to: Bład!
        '''
        self.status_text=wx.StaticText(self.panel, -1, "Błąd!", (380,120),size=(100, -1), style=wx.TE_CENTRE)
    ...
    def message_show(self, message):
        ...
        '''
        self.amounts()
        message_warning=wx.MessageDialog(None,message,'Błąd',wx.ICON_HAND)
    ...
    def amounts(self):
        ...
        try:
            ...
            amount_of_copied_cuts = 0
        self.amount_of_cuts_text.SetLabel("Ilość wszystkich przekrojów z bazy danych: " + str(amount_of_cuts))
        self.amount_of_copied_cuts_text.SetLabel("Ilość przekrojów jaka została skopiowana: " + str(amount_of_copied_cuts))
